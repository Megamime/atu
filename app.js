...
const THEME_KEY='megami_anime_theme';
...
const SK='animevault_v1';
const DB_NAME='ReikaDB', DB_VER=2, STORE='series', LOG_STORE='activityLog';
let db=null;
function openDB(){
...
// sınırlıydı (ma_log_<id>). Artık IndexedDB'de, sınır olmadan, tüm geçmiş boyunca saklanıyor.
...
async function addLog(seriesId,text,ts){
  ...
  try{
    ...
    await new Promise((res,rej)=>{req.onsuccess=res;req.onerror=rej;});
  }catch(e){ console.error('[Reika] Aktivite kaydı yazılamadı:',e); }
}
...
let series=[],editingId=null,currentPage='home',currentCat='all',searchQ='',currentSort=localStorage.getItem('megami_anime_sort')||'default',currentDetailId=null;
...
let fansubMeta={};
try{ fansubMeta=JSON.parse(localStorage.getItem('megami_anime_fansub_meta')||'{}'); }catch(e){ fansubMeta={}; }
function saveFansubMeta(){ try{ localStorage.setItem('megami_anime_fansub_meta',JSON.stringify(fansubMeta)); }catch(e){} }
...
function updateStreak(){
  ...
  let data=JSON.parse(localStorage.getItem('ma_streak')||'{}');
  if(data.last===today){}
...
function showStreak(){
  const data=JSON.parse(localStorage.getItem('ma_streak')||'{}');
...
function triggerKonamiEffect(){
  ...
  spawnConfetti();
  const msgs=[
    '✨ Gizli modu buldun! İzleme ruhu seninle! 📺',
    '🐉 7 kez tıkladın... Bu sabır bir anime kahramanına yakışır!',
    '🌸 Gizli bahçeyi keşfettin! Tebrikler!',
    "📖 Reika'nın kalbini açtın!"
  ];
...
function easterEggCheck(){
  ...
  const completedMsgs={
    5:  ['check','5 seri bitirdin! İlk rozet: "Anime Başlangıcı" 🥉'],
    10: ['star','10 seri bitirdin! Rozet: "Sayfaları Fırçalayan" 🥈'],
    25: ['star','25 seri bitirdin! Rozet: "Anime Koleksiyoncusu" 🥇'],
    50: ['star','50 seri bitirdin! Efsane seviye: "Anime Tanrısı" 👑'],
  };
...
function checkMidnightEgg(){
  ...
  if(h>=0&&h<4){
    const msgs=isLight?[
      ...
      'Bu saatte hâlâ izliyorsun, en azından temanı karart 👀',
      '🦉 Gece kuşu modu aktif! Uyumak yok, anime var!',
    ]:[
      'Gece yarısı anime mi izliyorsun? 🌙 Uyku düşmanı!',
      '🦉 Gece kuşu modu aktif! Uyumak yok, anime var!',
    ...
    const msgs=[
      '☀️ Sabahın köründe anime mi? Efsanesin!',
      '🐓 Horoz ötmeden önce buradasın, saygılar!',
      '☕ Kahve mi anime mi, ikisi de olsun?',
      '🌅 Gün doğumunu anime ile karşılamak farklı bir keyif.',
    ];
...
const TAB_USAGE_KEY='megami_anime_tab_usage';
...
function setSort(val){
  currentSort=val;
  localStorage.setItem('megami_anime_sort',val);
  renderContent();
}
...
// ===== Günlük Özet — "Reika'yı açtığında" görünen, bildirim tarzı bir kart: bugün yeni
// bölüm gelen seriler, sezon arasından dönenler, ve güncel olmayıp izlemeye devam ettiğin
...
function dismissDailyDigest(){
  localStorage.setItem('megami_anime_digest_dismissed',new Date().toDateString());
...
function buildDailyDigestHTML(){
  if(localStorage.getItem('megami_anime_digest_dismissed')===new Date().toDateString()) return '';
  const newCh=getTodaysNewChapters();
  const returned=getReturnedFromHiatus();
  const reading=getUnfinishedReading();
    ...
    return `<div class="hero-slide" onclick="openPreview('${s.id}',event)">
      ...
      <div class="hero-content">
        ...
        <div class="hero-actions">
          <div class="hero-cta" onclick="event.stopPropagation();${s.readUrl?`window.open('${esc(s.readUrl)}','_blank')`:`openQuick('${s.id}')`};"><svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor"><polygon points="6 3 20 12 6 21 6 3"/></svg>${isNew?'Yeni Bölümü İzle':'Kaldığın Yerden İzle'}</div>
...
function carouselCard(s,i,canReorder){
  ...
  const cover=s.cover
    ?`<img class="card-cover" src="${esc(s.cover)}" loading="lazy" onerror="console.warn('[Reika] Kapak yüklenemedi:', this.src);this.style.display='none';this.nextElementSibling.style.display='flex'">${coverLetterPh(s,true)}`
...
// açılan hafif bir ön izleme. Favori/İzle/Sabitle + hızlı bölüm ilerletme burada; "Durum
...
function openPreview(id,ev){
  ...
  document.getElementById('previewBody').innerHTML=`
    ...
    <div class="preview-row1">
      ...
      <div class="preview-read-btn" onclick="${s.readUrl?`window.open('${esc(s.readUrl)}','_blank')`:`openQuick('${id}')`};event.stopPropagation();">${ic('bolt',14)} İzle</div>
...
function getSeriesDetailSections(s){
  ...
  if(mainLinkedSeries.length||hasOwnSideLinks){
    ...
    const cov=ls=>ls.cover?`<img src="${esc(ls.cover)}" class="detail-htree-cover-img">`:`<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.4"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>`;
    ...
    linksH=`<div><div class="detail-sec-title">${ic('layers',10)} Bağlantılı Seriler</div>
      <div class="detail-htree" style="padding-top:${padTop}px;">
        ${branchesH}
        <div class="detail-htree-row">${rowH}</div>
      </div>
    </div>`;
  }
  ...
  el.innerHTML=`
    ...
    <div class="detail-cover-wrap">
      ...
      <div class="detail-badges-row">
        ...
        ${s.favorited?`<span class="detail-pill" style="background:var(--favG);color:var(--fav);">${ic('heartFill',9)} Favori</span>`:''}
      </div>
    ...
    <div class="detail-body">
      ...
      <div class="detail-action-row">
        ...
        <div class="action-chip ${s.favorited?'chip-fav':''}" onclick="toggleFav('${s.id}')">${s.favorited?ic('heartFill',13):ic('heart',13)} ${s.favorited?'Favoriden Çıkar':'Favorile'}</div>
      </div>
...
function checkFavoriteMilestone(){
  ...
  const msgs={5:'⭐ 5 favori! Zevkin belli oluyor.',15:'🌟 15 favori! Seçici bir izleyicisin.',30:'💫 30 favori! Favori listen de bir kütüphane oldu.'};
...
async function saveSeries(){
  ...
  try{
    ...
    console.error('[Reika] Kaydetme başarısız:',err);
...
if('serviceWorker' in navigator){
  window.addEventListener('load',()=>{
    ...
    }).catch(err=>console.warn('[Reika] Service worker kaydı başarısız:',err));
...
const PROFILE_RANKS=[
  {min:0,  label:'Çaylak İzleyici'},
  {min:10, label:'Bölüm Avcısı'},
  {min:25, label:'Anime Tutkunu'},
  {min:50, label:'Kütüphane Efendisi'},
  {min:100,label:'Fansub Bilgesi'},
  {min:200,label:'Efsanevi İzleyici'},
];
let _profileAvatarPending='✨';
function getProfile(){
  let p={};
  try{ p=JSON.parse(localStorage.getItem('megami_anime_profile')||'{}'); }catch(e){}
...
function saveProfileData(p){ localStorage.setItem('megami_anime_profile',JSON.stringify(p)); }
...
function openProfileSheet(){
  ...
  // Not: rütbe kavramı tasarımdan tamamen kaldırıldı (İstatistik sayfasında da yok).
  // Üyelik tarihi burada duruyor çünkü İstatistik sayfasında zaten olan sayıları
  // ("Toplam Seri"/"Bitirilen" gibi) burada TEKRARLAMIYORUZ, bu gereksiz olurdu.
  document.getElementById('profileStatsGrid').innerHTML=`
    <div class="profile-stat-card" style="grid-column:1/-1;"><div class="profile-stat-num" style="font-size:13px;">${esc(getMemberSinceLabel())}</div><div class="profile-stat-label">Üyelik</div></div>
  `;
...
async function exportBackup(){
  ...
  a.href=url; a.download=`animevault-yedek-${date}.json`;
  a.click(); URL.revokeObjectURL(url);
  localStorage.setItem('megami_anime_last_backup',JSON.stringify({ts:Date.now(),count:series.length,size:blob.size}));
  updateBackupInfo();
...
function updateBackupInfo(){
  ...
  if(!el) return;
  let last=null;
  try{ last=JSON.parse(localStorage.getItem('megami_anime_last_backup')||'null'); }catch(e){}
...
function normalizeCoverUrl(u){
  ...
  console.warn('[Reika] Geçersiz kapak URL formatı, kaydedilmedi:',u);
  return '';
}
// ═══════════════════════════════════════════
// RADIAL MENU
